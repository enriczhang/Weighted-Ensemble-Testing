...
type = 's';      %u = uniform binning+alloc, a = h-adaptive binning+v-alloc
num_bins = 6;	 %can become input variable
if type == 'c'
...
bin_floor = 1; %sets the minimum allocation per occupied bin
...
if type == 'g'
    ...
    W=reshape(Z,2,[]);
    H=h_(W);
    [~, sort_h_ind] = sort(h); %~ ignores the output like _ in python
    ...
    new_lvls_spacing = (cs_muv(end) - cs_muv(1)) / (num_bins+1);
    levels = (linspace(1,num_bins,num_bins)*new_lvls_spacing)';
elseif type == 's'
    xgrd = linspace(0.01,0.99,100);
    [X,Y] = meshgrid(xgrd,xgrd);
    W=combvec(xgrd,xgrd);
