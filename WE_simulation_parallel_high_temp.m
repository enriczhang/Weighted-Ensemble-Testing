...
loadname = sprintf('HT_initialization_data_%s_%d_%d.mat',WE_type,Bin_Floor,init_pop);
if rerun == 0
    try
        load(loadname)
    catch
        WE_live_init_high_temp(WE_type,Bin_Floor,init_pop,max_bins);
        load(loadname)
    end
else
    WE_live_init_high_temp(WE_type,Bin_Floor,init_pop,max_bins);
...
savename = sprintf('HT1_WE_data_%s_%d_%d.mat',WE_type,Bin_Floor,init_pop);
save(savename)
end %population loop
end %WE_type loop
